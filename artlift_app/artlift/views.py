...
class ArtistCreateView(APIView):
    ...
    def post(self, request):
        ...
        img_url = upload_to_supabase_profile(img_file) if img_file else ""
        bg_url = upload_to_supabase_profile(bg_file) if bg_file else ""
...
def upload_to_supabase_profile(file):
    filename = f"{uuid.uuid4()}.{file.name.split('.')[-1]}"
    content_type, _ = mimetypes.guess_type(filename)

    supabase.storage.from_("artist-profiles").upload(
        path=filename,
        file=file.read(),
        file_options={"content-type": content_type},
    )
